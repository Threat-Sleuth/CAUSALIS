[README_CAUSALIS_v0.8.1.md](https://github.com/user-attachments/files/30448305/README_CAUSALIS_v0.8.1.md)
# CAUSALIS v0.8.1
...
Version 0.8.0 preserves the validated CAUSALIS v0.6.0 experimentation core and adds targeted reliability and usability improvements discovered during the final doctoral-thesis campaign.
...
## Persistent CAPEC multiclass training

CAPEC training now runs through the persistent Task Manager. The input CSV files are copied into the task directory before execution. Results are checkpointed after every model, resources are released before the next fit, and an interrupted task can be resumed without repeating completed models.

A generic estimator cannot be checkpointed in the middle of `fit()`. Therefore, an interruption during a model fit repeats only that in-progress model; previously completed models are preserved.
...
CAUSALIS v0.8.1 treats the value as UTC internally:
...
See `RELEASE_NOTES_CAUSALIS_v0.8.1.md` and `CHANGELOG.md` for release details.
